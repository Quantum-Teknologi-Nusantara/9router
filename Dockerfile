...
# --chown on each COPY, not a `chown -R /app` afterwards: that rewrites every
# file into a second full layer (~9 min of build and a doubled push).
...
RUN mkdir -p /app/data /app/data-home && chown node:node /app /app/data /app/data-home && \
